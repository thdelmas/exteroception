...
- **Clocks** — deadlines, renewal dates, decision points within the horizon (e.g. 7 days). An expired clock outranks everything else in the brief. `scripts/clock-sweep.sh` captures this in one pass, and captures it **from the owning docs, not from the dashboard**: every dated obligation is written down where it belongs (the money doc holds the filing deadline, the decision doc holds the decision date), and reaches a dashboard only if some session pushed it there by hand. That push is manual, so the dashboard is complete only by luck — and its fresh date launders the items it never received. The sweep reads the owning docs directly and marks `UNSURFACED` any due date the merged clock doc doesn't carry. Past dates are provenance stamps, not clocks, and are dropped by construction.
